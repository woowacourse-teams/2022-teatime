ifndef::snippets[]
:snippets: ./build/generated-snippets
endif::[]
:doctype: book
:icons: font
:source-highlighter: highlightjs
:toc: left
:toclevels: 4

= 티타임 API 명세서

== 코치 API

=== 코치 저장
==== request
include::{snippets}/create-coach/http-request.adoc[]
==== response
include::{snippets}/create-coach/http-response.adoc[]

=== 코치 목록 조회
==== request
include::{snippets}/find-all-coaches/http-request.adoc[]
==== response
include::{snippets}/find-all-coaches/http-response.adoc[]

=== 코치 면담 목록 조회
==== request
include::{snippets}/find-coach-reservations/http-request.adoc[]
==== response
include::{snippets}/find-coach-reservations/http-response.adoc[]

=== 코치 히스토리 조회
==== request
include::{snippets}/find-own-history/http-request.adoc[]
==== response
include::{snippets}/find-own-history/http-response.adoc[]

== 스케줄 API

=== 스케줄 업데이트
==== request
include::{snippets}/update-schedule/http-request.adoc[]
==== response
include::{snippets}/update-schedule/http-response.adoc[]

=== 스케줄 조회
==== request
include::{snippets}/find-schedules/http-request.adoc[]
==== response
include::{snippets}/find-schedules/http-response.adoc[]

== 예약 API

=== 예약하기
==== request
include::{snippets}/reserve/http-request.adoc[]
==== response
include::{snippets}/reserve/http-response.adoc[]

=== 예약 승인하기
==== request
include::{snippets}/reserve-approve/http-request.adoc[]
==== response
include::{snippets}/reserve-approve/http-response.adoc[]

=== 예약 취소하기
==== request
include::{snippets}/reserve-cancel/http-request.adoc[]
==== response
include::{snippets}/reserve-cancel/http-response.adoc[]

=== 예약 완료하기
==== request
include::{snippets}/reserve-finish/http-request.adoc[]
==== response
include::{snippets}/reserve-finish/http-response.adoc[]

== 크루 API

=== 크루가 자신의 히스토리 조회
==== request
include::{snippets}/find-own-reservations/http-request.adoc[]
==== response
include::{snippets}/find-own-reservations/http-response.adoc[]

=== 크루가 자신의 면담시트 조회
==== request
include::{snippets}/find-own-sheets/http-request.adoc[]
==== response
include::{snippets}/find-own-sheets/http-response.adoc[]

=== 크루가 자신의 면담시트 답변 수정
==== request
include::{snippets}/update-sheet-answer/http-request.adoc[]
==== response
include::{snippets}/update-sheet-answer/http-response.adoc[]

=== 코치가 크루의 히스토리 조회
==== request
include::{snippets}/find-crew-reservations/http-request.adoc[]
==== response
include::{snippets}/find-crew-reservations/http-response.adoc[]

=== 코치가 크루의 면담시트 조회
==== request
include::{snippets}/find-crew-sheets/http-request.adoc[]
==== response
include::{snippets}/find-crew-sheets/http-response.adoc[]
